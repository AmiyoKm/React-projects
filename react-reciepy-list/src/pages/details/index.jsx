  ...
  useEffect(() => {
    async function getRecipeDetails() {
      const response = await fetch(
        `https://forkify-api.herokuapp.com/api/v2/recipes/${id}?key=cefc3172-15c8-4b9e-b956-7e06374b1207`
      );
  ...
  return (
    <div className="container mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-10 ">
      ...
      <div className="flex flex-col  gap-3">
        <span className="text-sm text-cyan700 font-medium">
        ...
        <div>
          <button onClick={()=> handleAddToFavorite(recipeDetailsData?.recipe)}className="p-3 px-8 rounded-lg text-sm uppercase font-medium tracking-wider mt-3 inline-block shadow-md bg-black text-white">
           {
            favoritesList.findIndex(item => item.id===recipeDetailsData?.recipe?.id !== -1) ? "Add To Favorites"   : "Remove From Favorites"
           }
          </button>
        </div>
        <div>
